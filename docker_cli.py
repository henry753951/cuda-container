...
def deploy_container():
    ...
    subprocess.run(['docker', 'compose', 'up', '-p', os.environ['PROJECT_NAME'], '-d'], check=True, env=compose_env)

    # Initialize Docker client
    client = docker.from_env()

    # Get container information
    container_cs = client.containers.get(f'{answers["project_name"]}-cs')
    container_novnc = client.containers.get(f'{answers["project_name"]}-novnc')
